...
begin
    try
        with Memo1 do
        begin
              Line      := Perform(EM_LINEFROMCHAR, SelStart, 0) ;  //длина строки
              SelStart  := Perform(EM_LINEINDEX, Line, 0) ;         //выделение длины строки по индексу
              SelLength := Length(Lines[Line]);                     //индекс строки
